...
export const Route = createFileRoute("/feed")({
  ...
  head: () => ({
    meta: [
      { title: "My Items — FindIt AI" },
      { name: "description", content: "View and manage your reported items." },
    ],
  }),
...
function MyItemsPage() {
  ...
  const { data: items = [], isLoading } = useQuery({
    queryKey: ["items"],
    queryFn: () => api.get<ApiItem[]>("/api/items/"),
  });
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="mx-auto max-w-6xl px-4 py-10">
        <header className="mb-8 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            <p className="text-xs uppercase tracking-wider text-muted-foreground">My Items</p>
            <h1 className="mt-2 font-display text-4xl md:text-5xl">Manage your reports</h1>
            <p className="mt-2 max-w-lg text-sm text-muted-foreground">
              These are the items you have reported. The system constantly matches them against new reports.
            </p>
          </div>
        ...
        <section>
          {isLoading ? (
            <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
              ...
              <p className="mt-4 text-sm font-medium">Loading your items...</p>
            </div>
              ...
              {filteredItems.map((item) => (
                <div key={item.id} className="relative group/card">
                  ...
                  <button
                    ...
                    className="absolute top-2 right-2 z-10 hidden group-hover/card:flex items-center justify-center p-2 bg-destructive text-destructive-foreground rounded-full shadow-sm hover:bg-destructive/90 transition-colors"
                    title="Delete item"
                  >
